...
- Uses Firebase Auth, Firestore, and Storage as the live data system.
- Runs as a static web app with no build step.
- Includes a local empty preview mode behind a preview admin password.
- Uses Firestore `admins/{uid}` documents for admin write access.
...
This only works with `?nofirebase=1`. Production uses Firebase Auth and the included `firestore.rules` pattern.

## Data Source

Firestore is the source of truth. The original Excel workbook was used only for the first import and is no longer loaded by the app. New edits are saved to Firebase.
...
The included Firestore rules require sign-in for reads and admin-only writes, where admins are represented by documents in `admins/{uid}`.
...
After that, sign in through the app. Admin edits write directly to Firestore.
